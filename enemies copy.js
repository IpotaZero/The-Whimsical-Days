...
enemy_data.zako_0 = new Enemy(null, 16, 30)
  .addf((me) => {
    ...
    if (me.frame % [36, 24, 12, 6][difficulty] == 0) {
      bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("p", me.p, "v", new vec([6, 12, 12, 12][difficulty], 0)).aim(player.p).nway([1, 3, 7, 11][difficulty], Math.PI / 12).bullets)
...
enemy_data.zako_2 = new Enemy(null, 32, 200)
  ...
  .addf((me) => {
    if (24 < me.frame && me.frame < 144 && me.frame % [24, 12, 6, 3][difficulty] == 0) {
      bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("p", me.p, "v", new vec([6, 12, 12, 12][difficulty], 0)).aim(player.p).nway(7, Math.PI / 16).bullets)
      Sound_Data.bullet0.play()
    }
...
for (let i = 0; i < 4; i++) {
  enemy_data["zako_3_" + (2 * i)] = new Enemy(new vec(game_width, game_height / 8 * i + 20), 16, 30)
    .addf((me) => {
      ...
      if (me.frame % [36, 24, 12, 6][difficulty] == 0) {
        bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("app", "laser", "r", 2, "p", me.p, "v", new vec([3, 6, 6, 6][difficulty], 0)).aim(player.p).arrow(20).bullets)
      }
  ...
  enemy_data["zako_5_" + i] = new Enemy(null, 16, 60)
    ...
    .addf((me) => {
      if (24 < me.frame && me.frame < 200 && me.frame % [48, 24, 18, 12][difficulty] == 0) {
        bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("p", me.p, "v", new vec([6, 12, 12, 12][difficulty], 0)).aim(player.p).sim(3, [3, 6, 6, 6][difficulty]).nway([3, 5, 7, 9][difficulty], Math.PI / 12).bullets)
...
enemy_data.zako_7 = new Enemy(null, 32, 695, { is_inv: true })
  ...
  .addf((me) => {
    ...
    bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("p", me.p, "v", new vec(12, 0)).ex([8, 12, 14, 16][difficulty]).bullets)
    Sound_Data.bullet1.play()

    if (me.frame % [24, 24, 12, 12][difficulty] == 0) {
      bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("life", 2, "r", 12, "p", me.p, "v", new vec(6, 0)).bound().aim(new vec(-player.p.x, player.p.y)).nway([1, 3, 5, 5][difficulty], Math.PI / 12).bullets)
      bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("life", 2, "r", 12, "p", me.p, "v", new vec(6, 0)).bound().aim(new vec(game_width * 2 - player.p.x, player.p.y)).nway([1, 3, 5, 5][difficulty], Math.PI / 12).bullets)
      Sound_Data.bullet0.play()
    }
...
enemy_data.ethanol_0 = new Enemy(new vec(game_width, game_height / 6), 56, 240, { is_boss: true })
  .addf((me) => {
    ...
    if (me.frame % 6 == 0) {
      bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("p", me.p, "v", new vec([6, 12, 12, 12][difficulty], 0)).ex([6, 12, 18, 24][difficulty]).rev(Math.PI / [6, 12, 18, 24][difficulty]).bullets)
      Sound_Data.bullet0.play()
    } else if (me.frame % 6 == 3) {
      bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("p", me.p, "v", new vec([6, 12, 12, 12][difficulty], 0)).ex([6, 12, 18, 24][difficulty]).bullets)
      Sound_Data.bullet0.play()
    }
...
enemy_data.ethanol_1 = new Enemy(null, 56, 600, { is_boss: true })
  ...
  .addf((me) => {
    if (me.frame < 48) {
      for (let i = 0; i < 7; i++) {
        bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("r", 6, "p", new vec(game_width / 2 + 60 * (i - 3), me.p.y - 60), "v", new vec(0, 1)).delete(1).arrow(64).bullets)
      }
    } else {
      me.p.x = game_width / 3 * Math.sin((me.frame - 48) * 2 * Math.PI / 120) + game_width / 2

      if (me.frame % [24, 16, 8, 4][difficulty] == 0) {
        bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("r", 15, "p", me.p, "v", new vec(0, -[3, 6, 6, 6][difficulty])).aim(player.p).nway(7, Math.PI / 24).bullets)
        Sound_Data.bullet1.play()
      }
    }
    ...
    .addf((me) => {
      if (me.frame > 48) {
        ...
        if (me.frame % [6, 4, 2, 1][difficulty] == 0) {
          bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("p", me.p, "v", new vec(0, -[3, 6, 6, 6][difficulty])).bullets)
          bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("p", me.p, "v", new vec(0, [3, 6, 6, 6][difficulty])).bullets)
          Sound_Data.bullet1.play()
        }
      }
...
enemy_data.ethanol_2 = new Enemy(null, 56, 600, { is_boss: true })
  ...
  .addf((me) => {
    if (me.frame > 48 && me.frame % [24, 24, 12, 12][difficulty] == 0) {
      bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("app", "laser", "r", 2, "p", me.p, "v", new vec(0, [3, 6, 6, 6][difficulty])).aim(player.p).nway([1, 3, 3, 3][difficulty], Math.PI / 12).arrow(30).bullets)
      bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("app", "laser", "r", 2, "p", me.p, "v", new vec(0, [3, 6, 6, 6][difficulty])).aim(player.p).do((me) => { me.v.y *= -1 }).nway([1, 3, 3, 3][difficulty], Math.PI / 12).arrow(30).bullets)
      Sound_Data.bullet0.play()
    }
  ...
  enemy_data["ethanol_3_" + i] = new Enemy(null, 32, 160)
    ...
    .addf((me) => {
      ...
      if (me.frame > 48) {
        ...
        if (me.frame % [24, 24, 12, 12][difficulty] == 0) {
          bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("app", "laser", "r", 2, "p", me.p, "v", new vec(0, [6, 12, 12, 12][difficulty])).aim(player.p).arrow(30).bullets)
        }
      }
...
enemy_data.ethanol_3 = new Enemy(null, 56, 600, { is_boss: true })
  ...
  .addf((me) => {
    if (me.frame > 48 && me.frame % [18, 12, 9, 6][difficulty] == 0) {
      bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("p", me.p, "v", new vec(0, [6, 12, 12, 12][difficulty])).aim(player.p).ex(32).bullets)
      Sound_Data.bullet0.play()
    }
    ...
    .addf((me) => {
      if (me.frame > 60) {
        ...
        if (me.frame % [12, 8, 4, 1][difficulty] == 0) {
          bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("p", me.p, "v", new vec(0, [6, 12, 12, 12][difficulty])).aim(player.p).bullets)
          Sound_Data.bullet1.play()
        }
      }
  ...
  .addf((me) => {
    if (me.frame > 60) {
      ...
      if (me.frame % [48, 36, 24, 12][difficulty] == 0) {
        bullets.push(...new Bullet([bullet_model]).colourful(me.frame).set("app", "laser", "r", 2, "p", me.p, "v", new vec(0, [6, 12, 12, 12][difficulty])).aim(player.p).arrow(30).ex([16, 24, 32, 40][difficulty], me.p).bullets)
        Sound_Data.bullet0.play()
      }
    }
...
enemy_data.ethanol_5 = new Enemy(null, 56, 3000, { is_boss: true })
  ...
  .addf((me) => {
    if (me.frame > 60) {
      if (me.frame % 2 == 0) {
        bullets.push(...new Bullet([bullet_model]).hakkyou_colourful(me.frame).set("p", me.p, "v", new vec(0, 6).rot(me.frame * 6 * Math.PI / 180)).ex(9).bullets)
        Sound_Data.bullet1.play()
      } else if (me.life < me.maxlife / 2) {
        bullets.push(...new Bullet([bullet_model]).hakkyou_colourful(me.frame - 120).set("p", me.p, "v", new vec(0, 6).rot(-me.frame * 6 * Math.PI / 180)).ex(9).bullets)
        Sound_Data.bullet1.play()
      }
    }
...
enemy_data.test3 = new Enemy(new vec(game_width / 2, 100), 56, 600, { is_boss: true })
  .addf((me) => {
    ...
    if (me.frame % 24 == 0) {
      bullets.push(...new Bullet([bullet_model]).set("p", me.p).set("v", new vec(12, 0)).bullets)
    }
...
const Bullet = class {
  constructor(bullets) {
    ...
    for (let i = 0; i < bullets.length; i++) {
      bullet_list.push({ ...bullets[i] });

      bullet_list[i].f = [];
      bullets[i].f.forEach((fun) => { bullet_list[i].f.push(fun); });
    }

    this.bullets = bullet_list
  }
  ...
  set(...key_value) {
    ...
    this.bullets.forEach((b) => {
      for (let i = 0; i < key_value.length / 2; i++) {
        b[key_value[2 * i]] = key_value[2 * i + 1]
      }
      c.push(b)
    })
  ...
  do(f) {
    ...
    this.bullets.forEach((b) => { f(b); c.push(b) })

    return new Bullet(c)
  }

  colourful(seed) {
    return this.set("colour", "hsl(" + Math.floor(90 * Math.sin(2 * Math.PI * seed / 240) + 90) + ",100%,50%)")
  }

  hakkyou_colourful(seed) {
    return this.set("colour", "hsl(" + Math.floor(90 * Math.sin(2 * Math.PI * seed / 240 + Math.PI / 2) - 40) + ",100%,50%)")
  }

  aim(p) {
    let c = []
    this.bullets.forEach((b) => { b.v = p.sub(b.p).nor().mlt(b.v.length()); c.push(b) })
  ...
  sim(num, max_speed) {
    ...
    this.bullets.forEach((b) => {
      ...
      for (let i = 0; i < num; i++) {
        c.push(...new Bullet([b]).set("v", b.v.nor().mlt(default_speed + (max_speed - default_speed) * i / (num - 1))).bullets)
      }
    })
  ...
  rev(angle, center = null) {
    let c = []

    if (center == null) {
      this.bullets.forEach((b) => { c.push(...new Bullet([b]).set("v", b.v.rot(angle)).bullets) });
    } else {
      this.bullets.forEach((b) => { c.push(...new Bullet([b]).set("p", b.p.sub(center).rot(angle).add(center)).set("v", b.v.rot(angle)).bullets) });
    }

    return new Bullet(c)
  }

  nway(num, interval, center = null) {
    let c = []

    const n = (num - 1) / 2;
    for (let i = 0; i < num; i++) {
      c.push(...this.rev(interval * (i - n), center).bullets);
    }
  ...
  ex(num, center = null) {
    return this.nway(num, 2 * Math.PI / num, center)
  }
  ...
  line(distance, points) {
    ...
    for (let i = 0; i < points.length - 1; i++) {
      let length = points[i + 1].sub(points[i]).length()
      for (let j = 0; j * distance < length; j++) {
        c.push(...this.set("p", points[i].add(points[i + 1].sub(points[i]).mlt(j * distance / length))).bullets)
      }
    }
  ...
  laser(size) {
    ...
    this.bullets.forEach((b) => {
      c.push(...new Bullet([b]).line(b.r, [b.p, b.p.add(b.v.nor().mlt(size))]).bullets)
    })
  ...
  arrow(size) {
    ...
    this.bullets.forEach((b) => {
      ...
      bul.push(...new Bullet([b]).set("v", new vec(1, 0)).laser(size).bullets)
      bul.push(...new Bullet([b]).set("p", b.p.add(new vec(1, 0).nor().mlt(size))).set("v", new vec(-1, -1)).laser(size / 2).bullets)
      bul.push(...new Bullet([b]).set("p", b.p.add(new vec(1, 0).nor().mlt(size))).set("v", new vec(-1, 1)).laser(size / 2).bullets)

      c.push(...new Bullet(bul).set("v", new vec(b.v.length(), 0)).rev(b.v.arg(), b.p).bullets)
    });
  ...
  wait(name, frame, f) {
    return this.set(name, 0).addf((me) => { if (me[name] == frame) { f(me); } me[name]++ })
  }

  delete(frame) {
    return this.wait("dead_line", frame, (me) => { me.life = 0 })
  }
