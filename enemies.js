...
enemy_data.tutorial_2 = new Enemy(null, 32, 200, { is_inv: true })
  ...
  .addf((me) => {
    ...
    if (me.frame % 12 == 0) {
      bullets.push(...remodel([bullet_model], ["r", 2, "app", "ball", "colourful", me.frame, "p", me.p, "v", new vec(0, 6), "sim", 3, 12]))
      Sound_Data.bullet0.play()
    }
...
enemy_data.phenetylalcohol_0 = new Enemy(new vec(game_width / 2, 100), 56, 600, { is_boss: true })
  .addf((me) => {
    if (me.frame % 2 == 0) {
      ...
      if (me.frame % 12 == 0) {
        bullets.push(...remodel([bullet_model], ["r", 2, "app", "ball", "colourful", me.frame, "r", 4, "p", me.p, "v", new vec(0, 12), "aim", player.p, "flower", 16, 5]))
      }
    }
...
for (let i = 0; i < 6; i++) {
  ...
  enemy_data["phenetylalcohol_2_" + i] = new Enemy(new vec(0, 80).rot(2 * Math.PI * i / 6).add(new vec(game_width / 2, game_height / 2)), 32, 200).addf(
    (me) => {
      if (me.frame % 3 == 0) {
        bullets.push(...remodel([bullet_model], ["colourful", me.frame, "p", me.p, "v", new vec(0, -12).rot(2 * Math.PI / num * i), "f", (me0) => {
...
enemy_data.phenetylalcohol_2 = new Enemy(new vec(game_width / 2, game_height / 2), 56, 900, { is_boss: true })
  .addf((me) => {
    if (me.frame % 48 == 0) {
      bullets.push(...remodel([bullet_model], ["app", "laser", "r", 0, "colourful", me.frame, "p", me.p, "v", new vec(0, 6), "aim", player.p, "frame", 0, "f", (me0) => {
        next_bullets.push(...remodel([bullet_model], ["app", "laser", "colour", me0.colour, "p", me0.p, "v", new vec(0, 6).rot(Math.PI * me0.frame / 60), "delete", 0, "cross", 20]))
...
enemy_data.phenetylalcohol_3 = new Enemy(new vec(game_width / 2, game_height / 2), 56, 1200, { is_boss: true })
  .addf((me) => {
    ...
    for (let i = 0; i < 5; i++) {
      bullets.push(...remodel([bullet_model], ["colourful", me.frame, "p", me.p, "v", new vec(0, 6).rot(2 * Math.PI * me.frame / 144 + 2 * Math.PI * i / 5), "wait", "frame", 32, (me0) => { me0.v = me0.v.mlt(-1) }, "delete", 64]))
    }

    if (me.frame % 24 == 0) {
      bullets.push(...remodel([bullet_model], ["r", 2, "app", "ball", "p", me.p, "v", new vec(0, 12), "aim", player.p, "ex", 19, me.p, "sim", 6, 4]))
    } else if (me.frame % 24 == 12) {
      bullets.push(...remodel([bullet_model], ["r", 2, "app", "ball", "p", me.p, "v", new vec(0, 12), "aim", player.p, "ex", 18, me.p, "sim", 6, 4]))
    }
...
enemy_data.phenetylalcohol_4 = new Enemy(new vec(game_width / 2, game_height / 2), 56, 1200, { is_boss: true })
  .addf((me) => {
    if (me.frame % 8 == 0) {
      bullets.push(...remodel([bullet_model], ["colourful", me.frame, "p", me.p, "v", new vec(0, 6), "frame", 0, "f", (me0) => {
        next_bullets.push(...remodel([bullet_model], ["p", me0.p, "v", me0.v, "delete", 1, "arrow", 60]))
...
enemy_data.phenetylalcohol_5 = new Enemy(new vec(game_width / 2, game_height / 6), 56, 1200, { is_boss: true })
  .addf((me) => {
    if (me.frame % 4 == 0) {
      bullets.push(...remodel([bullet_model], ["colourful", me.frame, "p", me.p, "v", new vec(0, 6).rot(Math.PI * me.frame / 13), "ex", 19, me.p]))
      bullets.push(...remodel([bullet_model], ["colourful", me.frame, "p", me.p, "v", new vec(0, 6).rot(-Math.PI * me.frame / 13), "ex", 19, me.p]))
