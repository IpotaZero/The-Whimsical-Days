...
const player_bullet = remodel(
  [bullet_model],
  ["r", 3, "app", "none", "colour", "#ffffff80", "type", "friend"]
)[0]
...
const scene_main = new (class extends Scene {
  ...
  set_score() {
    ...
    const a =
      save.data["stage_" + (this.chapter_num - 1)]["difficulty_" + difficulty]
      ...
      while (!["end", "wait", "sleep", "ok"].includes(element.type)) {
        switch (element.type) {
          ...
          case "score":
            ...
            const a =
              save.data["stage_" + (this.chapter_num - 1)][
              "difficulty_" + difficulty
              ]
            a["cleared"] = true
            if (player.life == 8) {
              a["no_miss_clear"] = true
            }
            this.set_score()
            break
  ...
  danmaku() {
    ...
    enemies.forEach((e) => {
      ...
      const damage =
        1 + Math.floor(3 * (1 - e.p.sub(player.p).length() / game_height))
...
const scene_title = new (class extends Scene {
  constructor() {
    ...
    this.option = {
      ...
      0: ["Tutorial", "Stage0", "Stage1"],
      "0.": ["Easy", "Normal", "Hard", "Insane!"],
    ...
    this.loopf = {
      0: (c) => {
        ...
        Itext5(
          ...
          400,
          font_size,
          ["Let's start here", "vs Ethanol", "Under Contruction..."][
          c.current_value
          ]
        )
      },
      "0.": (c) => {
        if (c.current_branch.charAt(1) == 0) {
          return
        }
        ...
        Itext5(
          ...
          font_size,
          "Highest Score: " +
          save.data["stage_" + stage]["difficulty_" + c.current_value][
          "highest_score"
          ]
        )
        Itext5(
      ...
      "3.": (c) => {
        ...
        Itext5(
          ...
          200,
          font_size,
          this.achive[c.current_branch.charAt(1)]
        )
      },
      4: (c) => {
        Itext4(
      ...
      ...Igenerator(function* () {
        for (let i = 0; i < 4; i++) {
          yield "Stage0の" +
            ...
            "をクリアする:<br>" +
            (save.data["stage_0"]["difficulty_" + i]["cleared"]
              ? "Achieved!"
              : "Unachived")
        }
      }),
      ...Igenerator(function* () {
        for (let i = 0; i < 4; i++) {
          yield "Stage0の" +
            ["Easy", "Normal", "Hard", "Insane!"][i] +
            "をノーミスクリアする:<br>" +
            (save.data["stage_0"]["difficulty_" + i]["no_miss_clear"]
              ? "Achieved!"
              : "Unachived")
        }
      }),
...
const scene_pretitle = new (class extends Scene {
  ...
  start() {
    ...
    $("body").append(
      "<input type='range' id=volume_bgm min=0 max=12 step=1 onchange='range(id)'></input>"
    )
    $("body").append("SE Volume:")
    $("body").append(
      "<input type='range' id=volume_se min=0 max=12 step=1 onchange='range(id)'></input>"
    )
...
const scene_gameover = new (class extends Scene {
  ...
  loop() {
    if (this.frame < 24) {
      ...
      Itext4(null, 0, game_height - font_size, font_size, [
        "[Z] to back to title",
        "[R] to retry",
      ])
...
const button = (id) => {
  ...
  switch (id) {
    ...
    case "control_mode":
      config.data.control_mode = { mouse: "key", key: "mouse" }[
        config.data.control_mode
      ]
      config.save()
